...
function renderMathDrillView(view) {
    ...
    if (view === 'menu') {
        ...
        const typeLabel = item.drillType === 'POT_ODDS' ? 'Pot Odds' : 'Bet Sizing';
        if (titleEl) titleEl.textContent = typeLabel;
        if (progEl)  progEl.textContent  = `${mathDrill.idx + 1} / ${mathDrill.queue.length}`;
        content.innerHTML = item.drillType === 'POT_ODDS'
            ? renderPotOddsQuestion(item.s)
...
function renderMenuView() {
    ...
    <div class="flex flex-col items-center gap-5 px-4 py-8 max-w-md mx-auto w-full">
        ...
        <div class="w-full flex flex-col gap-3">
            ...
            <button onclick="startMathDrill('POT_ODDS')"
                ...
                <div class="flex items-start justify-between mb-2">
                    <span class="text-base font-black text-slate-100">Pot Odds & Equity</span>
                    ${dueTag(poDue, (POT_ODDS_SCENARIOS||[]).length)}
                </div>
                <p class="text-[11px] text-slate-500 leading-snug">
                    Face a bet — decide if you have enough equity to call. Flush draws, straight draws, pair equity, river decisions.
                </p>
                <p class="text-[10px] text-indigo-400 font-bold mt-2">${(POT_ODDS_SCENARIOS||[]).length} scenarios · 6 categories</p>
            </button>

            <button onclick="startMathDrill('BET_SIZE')"
                class="w-full bg-slate-900 border border-slate-700 hover:border-indigo-500 active:scale-[0.98] rounded-2xl p-5 text-left transition-all">
                <div class="flex items-start justify-between mb-2">
                    <span class="text-base font-black text-slate-100">Bet Sizing</span>
                    ${dueTag(bsDue, (BET_SIZING_SCENARIOS||[]).length)}
                </div>
                <p class="text-[11px] text-slate-500 leading-snug">
                    You have a hand — pick the right size. Dry vs. wet boards, river value, bluffs, position adjustments.
                </p>
                <p class="text-[10px] text-indigo-400 font-bold mt-2">${(BET_SIZING_SCENARIOS||[]).length} scenarios · 7 categories</p>
            </button>
            ...
            <button onclick="startMathDrill('MIXED')"
                ...
                <div class="flex items-start justify-between mb-1">
                    ...
                    ${dueTag(mixDue, ((POT_ODDS_SCENARIOS||[]).length + (BET_SIZING_SCENARIOS||[]).length))}
                </div>
                <p class="text-[11px] text-slate-400">Both drill types in one session — recommended for daily practice.</p>
            </button>
        </div>

        <p class="text-[9px] text-slate-700 font-bold uppercase tracking-widest text-center">
            Spaced repetition · Sessions auto-prioritise your weak spots
        </p>
    </div>`;
}
...
function renderSummaryView() {
    ...
    const catRows = Object.values(cats).map(c => {
        ...
        const catMeta = c.drillType === 'POT_ODDS'
        ...
        const typeTag = c.drillType === 'POT_ODDS' ? 'PO' : 'BS';
    ...
    const weakLabel = weakest && weakest.correct < weakest.total
        ? ((weakest.drillType === 'POT_ODDS' ? POT_ODDS_CATEGORIES : BET_SIZING_CATEGORIES)
            ?.[weakest.category]?.label || weakest.category)
        : null;
